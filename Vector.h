 ...
 * Author: amin
 *
 * Created on March 3, 2016, 2:14 PM
 */
...
class Vector {
   public:
     ...
     Vector( Presicion newx, Presicion newy, Presicion newz) {
       x = newx;
       y = newy;
       z = newz;
     }
     ...
     //  v1 = const;
     Vector& operator=(const Presicion &v2) {
       x = v2;
       y = v2;
       z = v2;
       return *this;
     ...
     // v1 *= const
     void operator*=(const Presicion &v2) {
       x *= v2;
       y *= v2;
       z *= v2;
     }
     ...
     // inner ("dot") product
     friend Presicion operator*(const Vector &v1, const Vector &v2) {
       return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
     }
     // scalar product
     friend Vector operator*(const Presicion &f, const Vector &v1) {
       return Vector(f*v1.x, f*v1.y, f*v1.z);
     }
     // scalar product
     friend Vector operator*(const Vector &v1, const Presicion &f) {
       return Vector(f*v1.x, f*v1.y, f*v1.z);
     }
     ...
     // return the norm
     inline Presicion length(void) const {
       return sqrt(x*x+y*y+z*z);
     }
     
     Presicion length2(void) const {
       return (x*x + y*y + z*z);
     }
     ...
     inline friend Vector cross(const Presicion k, const Vector &v1, const Vector &v2) {
	...
	z+=c;
     ...
     void mult(Presicion f) {
       x*=f; y*=f; z*=f;
     }

     // divide each element by a scalar
     void div(Presicion f) {
       x/=f; y/=f; z/=f;
     }

     // returns (*this) * V2
     Presicion dot(const Vector &v2) {
	...
	Presicion a[3];    // read into Presicions, since I don't know what
	char tmp[100];  // a "Presicion" is in real life
